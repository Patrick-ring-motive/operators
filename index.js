...
try{
  ...
  if(e.message.toLowerCase().includes('illegal')){globalThis.isLegal = false;}
}
...
globalThis.undefine=function(obj,prop,val){
  Object.defineProperty(obj, prop, {value: val, writable: true,configurable:true,enumerable:false});
  return obj;
};
globalThis.overwriteAll=function(obj,src){
  let keys=Object.keys(src).concat(Object.keys(Object.getOwnPropertyDescriptors(src)));
  for(let i in src){
   keys.push(i);
  }
  keys=Array.from(new Set(keys));
  const keys_length=keys.length;
  for(let i=0;i<keys_length;i++){try{
...
globalThis.del=function(obj,prop){
  ...
  delete obj[prop];
  if(obj[prop]){undefine(obj,prop);}
  return obj;
}
globalThis.deleteAll=function(obj){
  let keys=Object.keys(obj).concat(Object.keys(Object.getOwnPropertyDescriptors(obj)));
  for(let i in obj){
   keys.push(i);
  }
  keys=Array.from(new Set(keys));
...
globalThis.OP = function(obj,param,op,val){
  ...
  switch(param){
    ...
    case '=??':
      if((op===null)||(op===undefined)){return obj;}
      return hardAssign(obj??{},op);
    case '||=':
